...
// An optional *sync.Pool may be provided; if nil a default 1920-byte PCM pool is created.
func NewAtomicRingBuffer(capacityPow2 uint64, pool ...*sync.Pool) *AtomicRingBuffer {
...
// DrainAndRelease acquires the buffer lock, nils every live slot between readIndex and
// writeIndex, then advances readIndex to writeIndex so the buffer appears empty.
// It does NOT interact with the pool; callers are responsible for pool lifecycle.
func (rb *AtomicRingBuffer) DrainAndRelease() {
	rb.mu.Lock()
	defer rb.mu.Unlock()
	...
	for i := read; i < write; i++ {
		rb.buffer[i&rb.mask] = nil
	}
	atomic.StoreUint64(&rb.readIndex, write)
}
...
func (s *DeterministicAudioStream) dispatchLoop() {
	// Capture the generation counter at goroutine start so stale goroutines
	// spawned before a Reload() call can self-terminate.
	gen := s.reloadGen.Load()
	...
	for {
		...
		case tickTime := <-ticker.C:
			// Self-terminate if this goroutine belongs to a previous reload generation.
			if s.reloadGen.Load() != gen {
				return
			}
...
// After cancellation it drains the ring buffer and nils remaining subscriber channels.
func (s *DeterministicAudioStream) Close() error {
	var err error
	s.closeOnce.Do(func() {
		atomic.StoreInt32(&s.isClosed, 1)
		s.cancel()
		s.ringBuffer.DrainAndRelease()
		close(s.outChan)
...
// Drain reads and discards all frames currently queued in the ring buffer,
// returning the number of frames drained. It is safe to call concurrently
// with Close().
func (s *DeterministicAudioStream) Drain() int {
	count := 0
	for {
		_, ok := s.ringBuffer.Read()
		if !ok {
			break
		}
		count++
...
// Reload atomically replaces the running dispatch goroutine with a fresh one,
// optionally updating the stream ID. It resets all counters and latency
// metrics, drains the ring buffer, and increments the reload generation so
// the previous goroutine self-terminates on its next tick.
//
// Reload returns an error if the stream has already been closed.
func (s *DeterministicAudioStream) Reload(ctx context.Context, newStreamID string) error {
	if atomic.LoadInt32(&s.isClosed) == 1 {
		return errors.New("cannot reload a closed DeterministicAudioStream")
	}

	// Cancel the old dispatch loop and give it a moment to observe cancellation.
	s.cancel()
	time.Sleep(20 * time.Millisecond)

	// Purge any buffered frames from the previous session.
	s.ringBuffer.DrainAndRelease()

	// Reset pipeline counters atomically.
	atomic.StoreUint64(&s.frameCounter, 0)
	atomic.StoreUint64(&s.dispatched, 0)
	atomic.StoreUint64(&s.dropped, 0)

	// Reset latency metrics under the metrics lock.
	s.metricsMu.Lock()
	s.latencySumMs = 0
	s.maxLatencyMs = 0
	s.latencyCount = 0
	...
	// Build a new context/cancel pair derived from the caller's context.
	newCtx, newCancel := context.WithCancel(ctx)
	s.ctx = newCtx
	s.cancel = newCancel

	// Increment generation so the old goroutine self-terminates on its next tick.
	s.reloadGen.Add(1)

	// Relaunch the dispatch goroutine.
	go s.dispatchLoop()

	// Optionally rename the stream.
	if newStreamID != "" {
