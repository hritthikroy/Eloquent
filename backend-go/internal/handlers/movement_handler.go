...
type AudioAdjustment struct {
	GainBoostDB         float64 `json:"gainBoostDB"`         // Gain boost in decibels (e.g. +1.5 dB when standing)
	NoiseReductionLevel string  `json:"noiseReductionLevel"` // "standard", "boosted", "adaptive"
	BufferHeadroomMs    int     `json:"bufferHeadroomMs"`    // Dynamic audio buffer latency padding (ms)
	AcousticPreset      string  `json:"acousticPreset"`      // "near_field_sitting", "far_field_standing", "motion_walking", "stationary_fallback"
	Timestamp           int64   `json:"timestamp"`
...
func (h *MovementHandler) HandleMovementEvent(event MovementEvent) AudioAdjustment {
	...
	// Handle degraded or unavailable camera mode
	...
	case "walking":
		...
		adj = AudioAdjustment{
			...
			BufferHeadroomMs:    45, // Add +25ms motion compensation headroom
...
// GetState returns a thread-safe copy of the current movement and adjustment posture.
...
// PostMovement handles HTTP POST /api/movement
func (h *MovementHandler) PostMovement(c *gin.Context) {
	var event MovementEvent
	if err := c.ShouldBindJSON(&event); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"success": false,
			"error":   "Invalid movement event payload: " + err.Error(),
		})
		return
	}
...
// GetMovementState handles HTTP GET /api/movement/state
func (h *MovementHandler) GetMovementState(c *gin.Context) {
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"state":   h.GetState(),
	})
