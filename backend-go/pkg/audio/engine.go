...
// adaptive backpressure to eliminate buffer overruns, linear resampling, and clock synchronization.
package audio
...
type EngineConfig struct {
	...
	SampleRate         uint32  `json:"sampleRate"`
	TargetSampleRate   uint32  `json:"targetSampleRate"` // Optional target rate for linear resampling
	Channels           uint16  `json:"channels"`
...
func DefaultEngineConfig() EngineConfig {
	...
	return EngineConfig{
		...
		SampleRate:         48000,
		TargetSampleRate:   0, // 0 indicates passthrough (no resampling)
		Channels:           1,
...
// ─────────────────────────────────────────────────────────────────────────────
// LinearResampler: High-Performance Sample Rate Conversion
// ─────────────────────────────────────────────────────────────────────────────

// LinearResampler provides fast sample rate conversion for 16-bit linear PCM audio.
type LinearResampler struct{}

// NewLinearResampler returns a new LinearResampler.
func NewLinearResampler() *LinearResampler {
	return &LinearResampler{}
}

// Resample converts 16-bit linear PCM audio from srcRate to dstRate for the specified channel count.
func (r *LinearResampler) Resample(data []byte, srcRate, dstRate uint32, channels uint16) ([]byte, error) {
	if len(data) == 0 {
		return nil, ErrEmptyPayload
	}
	if len(data)%2 != 0 {
		return nil, ErrInvalidAudioPacket
	}
	...
	totalSourceFrames := len(data) / bytesPerFrame
	if totalSourceFrames == 0 {
		return nil, ErrBufferTooShort
	}

	totalTargetFrames := int((uint64(totalSourceFrames) * uint64(dstRate)) / uint64(srcRate))
	if totalTargetFrames == 0 {
		totalTargetFrames = 1
	}

	outLen := totalTargetFrames * bytesPerFrame
	out := make([]byte, outLen)
	ratio := float64(srcRate) / float64(dstRate)

	for ch := 0; ch < int(channels); ch++ {
		chOffset := ch * 2
		for dstIdx := 0; dstIdx < totalTargetFrames; dstIdx++ {
			srcPos := float64(dstIdx) * ratio
			srcIdx := int(srcPos)
			frac := srcPos - float64(srcIdx)

			if srcIdx >= totalSourceFrames-1 {
				srcIdx = totalSourceFrames - 1
				frac = 0
			}

			idx1 := (srcIdx * bytesPerFrame) + chOffset
			s1 := int16(binary.LittleEndian.Uint16(data[idx1 : idx1+2]))

			var interpVal int16
			if frac == 0 || srcIdx >= totalSourceFrames-1 {
				interpVal = s1
			} else {
				idx2 := ((srcIdx + 1) * bytesPerFrame) + chOffset
				s2 := int16(binary.LittleEndian.Uint16(data[idx2 : idx2+2]))
				interpVal = int16(float64(s1)*(1.0-frac) + float64(s2)*frac)
			}

			outOffset := (dstIdx * bytesPerFrame) + chOffset
			binary.LittleEndian.PutUint16(out[outOffset:outOffset+2], uint16(interpVal))
		}
	}
...
// ─────────────────────────────────────────────────────────────────────────────
// AudioEngine: Parallel DSP Worker Engine
// ─────────────────────────────────────────────────────────────────────────────

// AudioEngine coordinates parallel DSP worker goroutines with zero-overrun backpressure,
// real-time PTS alignment, and dynamic sample rate conversion.
type AudioEngine struct {
	cfg              EngineConfig
	targetSampleRate atomic.Uint32
...
func NewAudioEngine(cfg ...EngineConfig) *AudioEngine {
	...
	if config.TargetSampleRate > 0 {
		engine.targetSampleRate.Store(config.TargetSampleRate)
	}
...
// SetTargetSampleRate updates target sample rate for real-time resampling (0 to disable).
func (e *AudioEngine) SetTargetSampleRate(rate uint32) {
	e.targetSampleRate.Store(rate)
}

// Synchronizer returns the attached ClockSynchronizer.
func (e *AudioEngine) Synchronizer() *ClockSynchronizer {
	return e.synchronizer
}

// Resampler returns the attached LinearResampler.
func (e *AudioEngine) Resampler() *LinearResampler {
	return e.resampler
}
...
// processFrame executes high-performance 16-bit PCM RMS/peak calculation,
// clock PTS alignment, optional linear resampling, and VAD gating.
func (e *AudioEngine) processFrame(frame *AudioFrame) {
	...
	// Align frame PTS and apply clock synchronization
	if e.synchronizer != nil {
		_, _ = e.synchronizer.AlignFrame(frame)
	}

	// Dynamic sample rate conversion if TargetSampleRate is configured
	targetRate := e.targetSampleRate.Load()
	if targetRate > 0 && targetRate != frame.SampleRate {
		resampled, err := e.resampler.Resample(frame.Data, frame.SampleRate, targetRate, frame.Channels)
		if err == nil && len(resampled) > 0 {
			frame.Data = resampled
			frame.SampleRate = targetRate
			dataLen = len(resampled)
		}
	}
...
func (e *AudioEngine) GetMetrics() EngineMetrics {
	...
	if e.synchronizer != nil {
		syncMetrics := e.synchronizer.GetMetrics()
		driftUs = syncMetrics.ClockDriftUs
		syncState = syncMetrics.State
		corrections = syncMetrics.DriftCorrections
	}
