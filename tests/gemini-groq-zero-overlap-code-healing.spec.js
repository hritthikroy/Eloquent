...
async function runAll() {
  ...
  await test("TextSanitizer correctly normalizes phonetic distortions for zero overlap & code healing", async () => {
  ...
  // 2. Intent Parsing
  await test("IntentParser detects Gemini-Groq zero-overlap & code-healing directive", async () => {
    const rawPrompt = "gemini and groq api buffring overlaping and present dual sol fix this issues with deep research and thay change thare aura and charm betwen them or other nural somthing overlaping on conversation need 0 overlaping for deep smouth work all the day tuktuk need power to fix his own code and also other agent need this power to fix all thare codes for faster lerning and fixing agents of the yeas do deep research and fix all the bugs";
  ...
  await test("MasterApiGateway enforces in-flight turn abort and unified aura calibration", async () => {
  ...
  await test("GeminiClient manages active requests and anchors unified aura with temperature 0.80", async () => {
  ...
  await test("JarvisManager implements atomic zero-overlap speaking mutex and calibration", async () => {
    ...
    assert(duration < 150, `speak() preemption must be instant (<150ms), took ${duration}ms`);
    assert.strictEqual(jarvisManager.isSpeaking, false);
  });

  // 6. Autonomous Code-Healing Cortex
  await test("AutonomousCodeHealingCortex runs codebase health audit and closed-form mathematical proof", async () => {
  ...
  await test("AgentMedicMeshCortex integrates zero-overlap and autonomous code-healing audit", async () => {
  ...
  await test("ActionRunner dispatches directive with 100% persona sovereignty across all agents", async () => {
    ...
    // Tuk Tuk
    const tuktukRes = await runner.handleAction(
      directiveText,
    ...
    // Vision
    const visionRes = await runner.handleAction(
      directiveText,
    ...
    // Friday
    const fridayRes = await runner.handleAction(
      directiveText,
    ...
    // DD
    const ddRes = await runner.handleAction(
      directiveText,
      { key: "dd", name: "DD" }
    );
  ...
  await test("LocalCognitiveBrain synthesizes grounded responses across all agents and Squad", async () => {
    const directiveText = "gemini and groq api buffring overlaping and present dual sol fix this issues with deep research and thay change thare aura and charm betwen them or other nural somthing overlaping on conversation need 0 overlaping for deep smouth work all the day tuktuk need power to fix his own code and also other agent need this power to fix all thare codes for faster lerning and fixing agents of the yeas do deep research and fix all the bugs";

    const tuktukResp = LocalCognitiveBrain.synthesizeResponse("tuktuk", "Tuk Tuk", directiveText, {}, "en");
    ...
    const visionResp = LocalCognitiveBrain.synthesizeResponse("vision", "Vision", directiveText, {}, "en");
    ...
    const fridayResp = LocalCognitiveBrain.synthesizeResponse("friday", "Friday", directiveText, {}, "en");
    ...
    const ddResp = LocalCognitiveBrain.synthesizeResponse("dd", "DD", directiveText, {}, "en");
    assert(ddResp.includes("bro"), "DD brain response must include 'bro'");
    assert(!ddResp.endsWith("?"), "Zero trailing question marks");

    const squadResp = LocalCognitiveBrain.synthesizeResponse("team", "Squad", directiveText, {}, "en");
